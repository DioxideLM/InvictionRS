...
public class Client extends RSApplet {
	...
	/**
	 * End of Index Repacking Methods
	 **/
	...
	private void drawConsoleArea() {
		if (consoleOpen) {
			for (int index = 0, positionY = 308; index < 17; index++, positionY -= 18) {
			...
			if (consoleMessages[0].length() <= 0) {
				consoleMessage("Type 'commands' for a list of commands or 'clear' to clear the console.", false);
			}
		}
	}
	...
	public void getConsoleCommands() {
		consoleMessage("'clear' - clears the consoles' history.", false);
		consoleMessage("'data' - toggles displaying data (fps, memory usage, etc).", false);
		consoleMessage("'getitemid' - displays item IDs.", false);
		consoleMessage("'getnpcanimations' - displays NPC animations.", false);
		consoleMessage("'exit' - closes the client completely.", false);
	}
	...
	private void sendCommandPacket(String command) {
		...
		case "data":
			...
			consoleMessage("Client Data: " + clientData, false);
			break;
		}
		...
		if (command.startsWith("getnpcanimations")) {
			String[] args = command.split(" ");
			if (args != null && args.length == 2) {
				EntityDef e = EntityDef.forID(Integer.parseInt(args[1]));
				consoleMessage("Stand animation: " + e.standAnim + ", Walk Animation: " + e.walkAnim, false);
			} else {
				consoleMessage("Invalid syntax; 'getnpcanimations npcID'.", false);
			}
		}
		if (command.startsWith("getitemid")) {
			if (command.length() > 9) {
				String name = command.substring(10);
				String matching = "";
				int matchCount = 0;
				for (int index = 0; index < ItemDef.totalItems; index++) {
					ItemDef item = ItemDef.forID(index);
					if (item.name.equalsIgnoreCase(name)) {
						matching += (matchCount != 0 ? ", " : "") + Integer.toString(index);
						matchCount++;
					}
				}
				consoleMessage("Total matching terms: " + matchCount + ".", false);
				if (matchCount > 0) {
					consoleMessage(matching, false);
				}
			} else {
				consoleMessage("Invalid syntax; 'getitemid itemID'.", false);
			}
		}
		String[] args = command.split(" ");
	...
	private void drawChatArea() {
		...
		if (messagePromptRaised) {
			...
			chatTextDrawingArea.drawText(0, "Enter Data:", 60, 259);
			chatTextDrawingArea.drawText(128, amountOrNameInput + "*", 80, 259);
			...
			for (int k = 0; k < 500; k++) {
				if (chatMessages[k] != null) {
					...
					if (s1 != null && s1.startsWith("@cr3@")) {
						s1 = s1.substring(5);
						byte0 = 20; // Junior Moderator
					}
					if (s1 != null && s1.startsWith("@cr4@")) {
						s1 = s1.substring(5);
						byte0 = 25; // (Senior) Moderator
					}
					if (s1 != null && s1.startsWith("@cr5@")) {
						s1 = s1.substring(5);
						byte0 = 30; // Head Moderator
					}
						...
						if (chatTypeView == 1 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210) {
								...
								case 10: // Premium
									...
									xPos += 14;
									break;
								case 20: // Junior Moderator
									modIcons[2].drawBackground(xPos + 1, yPos - 12);
									xPos += 14;
									break;
								case 25: // (senior) Moderator
									modIcons[3].drawBackground(xPos + 1, yPos - 12);
									xPos += 14;
									break;
								case 30: // Head Moderator
									modIcons[4].drawBackground(xPos + 1, yPos - 12);
									xPos += 14;
									break;
								case 35: // Admin
									modIcons[5].drawBackground(xPos + 1, yPos - 12);
									xPos += 14;
									break;
								case 50: // Owner
									modIcons[6].drawBackground(xPos + 1, yPos - 12);
									xPos += 14;
									break;

								}
								textDrawingArea.method385(0, s1 + ":", yPos, xPos + 1); // Messages
								                                                        // in
								                                                        // chatbox
						...
						if (chatTypeView == 2 || chatTypeView == 0) {
							if (yPos > 0 && yPos < 210) {
								...
								case 25: // (senior) Moderator
									...
									break;
								case 30: // Head Moderator
									modIcons[4].drawBackground(k1, yPos - 12);
									k1 += 12;
									break;
								case 35: // Admin
									modIcons[5].drawBackground(k1, yPos - 12);
									k1 += 12;
									break;
								case 50: // Owner
									modIcons[6].drawBackground(k1, yPos - 12);
									k1 += 12;
									break;
								}
			...
			switch (myPrivilege) {
			case 5: // Donator
				modIcons[0].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 10:// Premium
				modIcons[1].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 20:// Junior Moderator
				modIcons[2].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 25: // (Senior) Moderator
				modIcons[3].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 30: // Head Moderator
				modIcons[4].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 35: // Admin
				modIcons[5].drawBackground(xPos, 122);
				xPos += 14;
				break;
			case 50: // Owner
				modIcons[6].drawBackground(xPos, 122);
				xPos += 14;
				break;
			}
			textDrawingArea.method385(0, s + ":", 134, xPos);
			textDrawingArea.drawChatInput(255, xPos + textDrawingArea.getTextWidth(s + ": "), inputString + "*", 133, false);
	...
	// private Sprite[] statusGlobes = new Sprite[11];
	...
	/*
	 * private void drawStatusGlobes() { // Hit-points int hpPercent = 100 *
	 * currentStats[3] / maxStats[3]; statusGlobes[0].drawSprite(171, 11);
	 * statusGlobes[1].drawSprite(174, 14);
	 * statusGlobes[0].drawSpriteWithDimens(171, 11, 34 - (currentStats[3] * 34
	 * / maxStats[3])); if (hpPercent > 25) { statusGlobes[5].drawSprite(180,
	 * 22); } else { if (loopCycle % 20 <= 10) statusGlobes[5].drawSprite(180,
	 * 22); if (loopCycle % 20 <= 5) statusGlobes[6].drawSprite(180, 22); }
	 * smallText.drawCenteredText(calcGlobeTextColor(hpPercent), 215, ""+
	 * currentStats[3], 39, true); // Prayer int prayerPercent = 100 *
	 * currentStats[5] / maxStats[5]; statusGlobes[0].drawSprite(187, 50);
	 * statusGlobes[2].drawSprite(190, 53);
	 * statusGlobes[0].drawSpriteWithDimens(187, 50, 34 - (currentStats[5] * 34
	 * / maxStats[5])); statusGlobes[7].drawSprite(193, 57);
	 * smallText.drawCenteredText(calcGlobeTextColor(prayerPercent), 232, "" +
	 * currentStats[5], 78, true); // Run int runPercent = energy; if
	 * (statusGlobesHover == -1) { if (runIsOn == false) {
	 * statusGlobes[0].drawSprite(187, 89); statusGlobes[3].drawSprite(190, 92);
	 * statusGlobes[0].drawSpriteWithDimens(187, 89, 34 - (runPercent * 34 /
	 * 100)); statusGlobes[8].drawSprite(197, 95); } else if (runIsOn == true) {
	 * statusGlobes[0].drawSprite(187, 89); statusGlobes[4].drawSprite(190, 92);
	 * statusGlobes[0].drawSpriteWithDimens(187, 89, 34 - (runPercent * 34 /
	 * 100)); statusGlobes[9].drawSprite(197, 96); } } else if
	 * (statusGlobesHover == 0) { if (runIsOn == false) {
	 * statusGlobes[10].drawSprite(187, 89); statusGlobes[3].drawSprite(190,
	 * 92); statusGlobes[10].drawSpriteWithDimens(187, 89, 34 - (runPercent * 34
	 * / 100)); statusGlobes[8].drawSprite(197, 95); } else if (runIsOn == true)
	 * { statusGlobes[10].drawSprite(187, 89); statusGlobes[4].drawSprite(190,
	 * 92); statusGlobes[10].drawSpriteWithDimens(187, 89, 34 - (runPercent * 34
	 * / 100)); statusGlobes[9].drawSprite(197, 96); } }
	 * smallText.drawCenteredText(calcGlobeTextColor(runPercent), 232, "" +
	 * energy, 117, true); }
	 */
	...
	public void drawSideIcons() {
		...
		if (tabInterfaceIDs[14] != -1) {
			sideIcons[14].drawSprite(11, 303);
		}
	}
	...
	/*
	 * private void drawMenu() { //Old Menu int i = menuOffsetX; int j =
	 * menuOffsetY; int k = menuWidth; int l = menuHeight + 1; int i1 =
	 * 0x5d5447; //DrawingArea.drawPixels(height, yPos, xPos, color, width);
	 * //DrawingArea.fillPixels(xPos, width, height, color, yPos);
	 * DrawingArea.drawPixels(l, j, i, i1, k); DrawingArea.drawPixels(16, j + 1,
	 * i + 1, 0, k - 2); DrawingArea.fillPixels(i + 1, k - 2, l - 19, 0, j +
	 * 18); chatTextDrawingArea.method385(i1, "Choose Option", j + 14, i + 3);
	 * int j1 = super.mouseX; int k1 = super.mouseY; if(menuScreenArea == 0) {
	 * j1 -= 4; k1 -= 4; } if(menuScreenArea == 1) { j1 -= 519; k1 -= 168; }
	 * if(menuScreenArea == 2) { j1 -= 17; k1 -= 338; } if(menuScreenArea == 3)
	 * { j1 -= 519; k1 -= 0; } for(int l1 = 0; l1 < menuActionRow; l1++) { int
	 * i2 = j + 31 + (menuActionRow - 1 - l1) * 15; int j2 = 0xffffff; if(j1 > i
	 * && j1 < i + k && k1 > i2 - 13 && k1 < i2 + 3) j2 = 0xffff00;
	 * chatTextDrawingArea.method389(true, i + 3, j2, menuActionName[l1], i2); }
	 * }
	 */
	private void drawMenu() { // New Menu
		int i = menuOffsetX;
		int j = menuOffsetY;
		int contextWidth = menuWidth;
		int contextHeight = menuHeight;
		DrawingArea.drawPixels(contextHeight - 4, j + 2, i, 0x706a5e, contextWidth);
		DrawingArea.drawPixels(contextHeight - 2, j + 1, i + 1, 0x706a5e, contextWidth - 2);
		DrawingArea.drawPixels(contextHeight, j, i + 2, 0x706a5e, contextWidth - 4);
		DrawingArea.drawPixels(contextHeight - 2, j + 1, i + 3, 0x2d2822, contextWidth - 6);
		DrawingArea.drawPixels(contextHeight - 4, j + 2, i + 2, 0x2d2822, contextWidth - 4);
		DrawingArea.drawPixels(contextHeight - 6, j + 3, i + 1, 0x2d2822, contextWidth - 2);
		DrawingArea.drawPixels(contextHeight - 22, j + 19, i + 2, 0x524a3d, contextWidth - 4);
		DrawingArea.drawPixels(contextHeight - 22, j + 20, i + 3, 0x524a3d, contextWidth - 6);
		DrawingArea.drawPixels(contextHeight - 23, j + 20, i + 3, 0x2b271c, contextWidth - 6);
		DrawingArea.fillPixels(i + 3, contextWidth - 6, 1, 0x2a291b, j + 2);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x2a261b, j + 3);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x252116, j + 4);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x211e15, j + 5);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x1e1b12, j + 6);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x1a170e, j + 7);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 2, 0x15120b, j + 8);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x100d08, j + 10);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x090a04, j + 11);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x080703, j + 12);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x090a04, j + 13);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x070802, j + 14);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x090a04, j + 15);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x070802, j + 16);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x090a04, j + 17);
		DrawingArea.fillPixels(i + 2, contextWidth - 4, 1, 0x2a291b, j + 18);
		DrawingArea.fillPixels(i + 3, contextWidth - 6, 1, 0x564943, j + 19);
		...
		if (contextWidth == menuWidth && contextHeight == menuHeight) {
			for (int l1 = 0; l1 < menuActionRow; l1++) {
				...
				int j2;
				j2 = 0xc6b895;
				if (j1 > i && j1 < i + contextWidth && k1 > i2 - 13 && k1 < i2 + 3) {
	...
	private void resetLogout() {
		...
		loggedIn = false;
		loginScreenState = 0;
		if (Configuration.rememberDetails) {
			myUsername = Configuration.USERNAME;
			myPassword = Configuration.PASSWORD;
		} else {
			myUsername = "";
			myPassword = "";
		}
	...
	private void method46(int i, Stream stream) {
		while (stream.bitPosition + 21 < i * 8) {
			...
			npc.desc = EntityDef.forID(stream.readBits(14)); // tweet me lads
	...
	public static void main(String args[]) {
		try {
			...
			args[3] = "members";
			args[4] = "32";
			System.out.println("Initializing " + Configuration.SERVER_NAME + " Client.");
			if (args.length != 5) {
	...
	private void doAction(int i) {
		...
		if (l == 315) {
			...
			if (flag8) {
				...
				case 21341: // Show equipment screen
					// openInterfaceID = 21172;
					openInterfaceID = 15106;
					rollingCharacter = 0;
					rsInterface.modelRotation2 = 0;
					break;
				case 15215: // Turn Left [Equipment Interface]
					if (rollingCharacter == 0) {
						rollingCharacter = -1;
					} else {
						rollingCharacter = 0;
					}
					break;
				case 15216: // Turn Right [Equipment Interface]
					if (rollingCharacter == 0) {
						rollingCharacter = 1;
					} else {
						rollingCharacter = 0;
	...
	private void method73() {
		do {
			...
			if (j == 96) // this is the ` button on ur keyboard for the console,
				// we make this stop working so that we dont type
				// shit in our chatbox when opening/closing the
				// console
				break;
			if (consoleOpen) {
				...
				if ((j == 13 || j == 10) && consoleInput.length() > 0) {
					consoleMessage(consoleInput, true);
			...
			if (openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
				...
				if (j >= 32 && j <= 122 && amountOrNameInput.length() < 80) {
					amountOrNameInput += (char) j;
					inputTaken = true;
				}
				if (j == 8 && amountOrNameInput.length() > 0) {
				...
				if (j == 13 || j == 10) {
					if (amountOrNameInput.length() > 0) {
						stream.createFrame(60);
						stream.writeQWord(TextClass.longForInput(amountOrNameInput));
					}
					inputDialogState = 0;
					inputTaken = true;
				}
	...
	private void drawFriendsListOrWelcomeScreen(RSInterface class9) {
		...
		/*
		 * if (j == 328) { RSInterface rsInterface = class9; int verticleTilt =
		 * 150; int animationSpeed = (int) (Math.sin(loopCycle / 40D) * 256D) &
		 * 0x7ff; rsInterface.modelRotation1 = verticleTilt;
		 * rsInterface.modelRotation2 = animationSpeed; if (aBoolean1031) {
		 * Model characterDisplay = myPlayer.method452(); for (int l2 = 0; l2 <
		 * 5; l2++) { if (anIntArray990[l2] != 0) {
		 * characterDisplay.method476(anIntArrayArray1003[l2][0],
		 * anIntArrayArray1003[l2][anIntArray990[l2]]); if (l2 == 1) {
		 * characterDisplay.method476(anIntArray1204[0],
		 * anIntArray1204[anIntArray990[l2]]); } } } int staticFrame =
		 * myPlayer.anInt1511; characterDisplay.method469();
		 * characterDisplay.method470
		 * (Animation.anims[staticFrame].anIntArray353[0]); //
		 * characterDisplay.method479(64, 850, -30, -50, -30, true);
		 * rsInterface.anInt233 = 5; rsInterface.mediaID = 0;
		 * RSInterface.method208(aBoolean994, characterDisplay); } return; }
		 */
		if (j == 328) {
			RSInterface rsInterface = class9;
			rsInterface.modelZoom = 550;
			// rsInterface.modelRotation1 = 0;
			rsInterface.modelRotation1 = 100;
			if (rollingCharacter == 1) { // Right
				if (rsInterface.modelRotation2 >= 0)
					rsInterface.modelRotation2 -= 10;
				if (rsInterface.modelRotation2 < 0)
					rsInterface.modelRotation2 = 2047;
			}
			if (rollingCharacter == -1) { // Left
				if (rsInterface.modelRotation2 <= 2047)
					rsInterface.modelRotation2 += 10;
				if (rsInterface.modelRotation2 > 2047)
					rsInterface.modelRotation2 = 0;
			} else if (rollingCharacter == 0) {
				if (rsInterface.modelRotation2 != 0) {
	...
	private void drawSplitPrivateChat() {
		...
		for (int j = 0; j < 100; j++) {
			if (chatMessages[j] != null) {
				...
				if (s != null && s.startsWith("@cr3@")) {
					s = s.substring(5);
					byte1 = 20; // Junior Moderator
				}
				if (s != null && s.startsWith("@cr4@")) {
					s = s.substring(5);
					byte1 = 25; // (Senior) Moderator
				}
				if (s != null && s.startsWith("@cr5@")) {
					s = s.substring(5);
					byte1 = 30; // Head Moderator
				}
				if (s != null && s.startsWith("@cr6@")) {
					s = s.substring(5);
					byte1 = 35; // Admin
				...
				if ((k == 3 || k == 7) && (k == 7 || privateChatMode == 0 || privateChatMode == 1 && isFriendOrSelf(s))) {
					...
					case 25: // (senior) Moderator
						...
						break;
					case 30: // Head Moderator
						modIcons[4].drawBackground(k1, l - 12);
						k1 += 12;
						break;
					case 35: // Admin
						modIcons[5].drawBackground(k1, l - 12);
						k1 += 12;
						break;
					case 50: // Owner
						modIcons[6].drawBackground(k1, l - 12);
						k1 += 12;
						break;
	...
	void startUp() {
		...
		try {
			...
			/*
			 * try { SpriteLoader.loadSprites(streamLoader); cacheSprite =
			 * SpriteLoader.sprites; } catch (Exception e) {
			 * System.err.println("Error loading sprite archive!"); }
			 */
			...
			OverLayFlo474.unpackConfig(streamLoader);
			...
			if (Configuration.autoLogin) {
				login(Configuration.USERNAME, Configuration.PASSWORD, true);
			}
			return;
		} catch (Exception exception) {
								...
								if (k5 > DrawingArea.topX - 32 && k5 < DrawingArea.bottomX && j6 > DrawingArea.topY - 32 && j6 < DrawingArea.bottomY || activeInterfaceType != 0 && anInt1085 == i3) {
									...
									if (class30_sub2_sub1_sub1_2 != null) {
										...
										if (class30_sub2_sub1_sub1_2.anInt1444 == 33 || class9_1.invStackSizes[i3] != 1) {
											...
											// + k6);
											if (k10 >= 1) { // Green/White of
											                    // 100k+/10m+
												smallText.method385(0, intToKOrMil(k10), j6 + 10 + j7, k5 + k6 + 1);
	...
	private void drawMinimap() {
		...
		// aClass30_Sub2_Sub1_Sub1_1263.method352(152, i, anIntArray1229, 256 +
		// minimapInt3, anIntArray1052, l2, 9, 32, 152, j);
		compass.method352(33, minimapInt1, anIntArray1057, 256, anIntArray968, 25, 0, 0, 33, 25);
	...
	private void drawLoginScreen(boolean flag) {
		...
		if (loginScreenState == 2) {
			...
			j += 15;
			chatTextDrawingArea.method389(true, c / 2 - 88, 0xffffff, "Server IP: " + server + ((loginScreenCursorPos == 2) & (loopCycle % 40 < 20) ? "@yel@|" : ""), j);
			j += 15;
			if (!flag) {
				int i1 = c / 2 - 80;
				int l1 = c1 / 2 + 50;
	...
	public RSFont newSmallFont, newNormalFont, newRegularFont, newBoldFont, bold, fancy, regular, small;
	public TextDrawingArea normalFont, boldFont, fancyFont, regularText;
	public int rights;
	public String name;
	...
	private int rollingCharacter = 0;
