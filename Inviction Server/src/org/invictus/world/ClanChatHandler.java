...
public class ClanChatHandler {
	...
	public void updateClanChat(int clanId) {
		for (int j = 0; j < clans[clanId].members.length; j++) {
			...
			if (PlayerHandler.players[clans[clanId].members[j]] != null) {
				...
				c.getPlayerAssistant().sendString("Talking in: " + clans[clanId].name, 18139);
				c.getPlayerAssistant().sendString("Owner: " + clans[clanId].owner, 18140);
				...
				for (int i = 0; i < clans[clanId].members.length; i++) {
					if (clans[clanId].members[i] > 0) {
						if (PlayerHandler.players[clans[clanId].members[i]] != null) {
							c.getPlayerAssistant().sendString(PlayerHandler.players[clans[clanId].members[i]].playerName, slotToFill);
							slotToFill++;
						}
					}
				}
				for (int k = slotToFill; k < 18244; k++)
					c.getPlayerAssistant().sendString("", k);
			}
		}
	}
	...
	public void handleLootShare(Client c, int item, int amount) {
		sendLootShareMessage(c.clanId, c.playerName + " has received " + amount + "x " + org.invictus.model.items.Item.getItemName(item) + ".");
	}
