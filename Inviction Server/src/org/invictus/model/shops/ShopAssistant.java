...
/**
 * A class for handling all of the shop related functions on the server.
 * 
 * @author Joe
 *
 */
...
public class ShopAssistant {
	...
	public void openShop(int ShopID) {
		...
		c.isShopping = true;
		c.myShopId = ShopID;
		c.getPlayerAssistant().sendInventoryOverlay(3824, 3822);
		c.getPlayerAssistant().sendString(ShopHandler.ShopName[ShopID], 3901);
	}
		...
		synchronized (c) {
			...
			for (int i = 0; i < ShopHandler.ShopItems.length; i++) {
				if (ShopHandler.ShopItems[ShopID][i] > 0 || i <= ShopHandler.ShopItemsStandard[ShopID]) {
					...
					if (ShopHandler.ShopItems[ShopID][i] > ItemConfiguration.ITEM_AMOUNT || ShopHandler.ShopItems[ShopID][i] < 0) {
						ShopHandler.ShopItems[ShopID][i] = ItemConfiguration.ITEM_AMOUNT;
					}
	...
	public void sellToShopPrice(int removeId, int removeSlot) {
		for (int i : ItemConfiguration.ITEMS_SELLABLE) {
			if (i == removeId) {
	...
	public boolean sellItem(int itemID, int fromSlot, int amount) {
		...
		if (c.playerRights == 2 && !Configuration.ADMINISTRATOR_SELLING) {
	...
	public void setupSkillCapes(int capes, int capes2) {
		synchronized (c) {
			...
			c.myShopId = 14;
			c.getPlayerAssistant().sendInventoryOverlay(3824, 3822);
			c.getPlayerAssistant().sendString("Skillcape Shop", 3901);
