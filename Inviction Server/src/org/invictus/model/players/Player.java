...
import org.invictus.model.npcs.NPC;
import org.invictus.model.npcs.NPCHandler;
import org.invictus.util.ISAACRandomGen;
...
public abstract class Player {
	 ...
	 * Longs
	 */
	public long lastPlayerMove, lastPoison, lastPoisonSip, poisonImmune, lastSpear, lastProtItem, dfsDelay, lastVeng, lastYell, teleGrabDelay, protMageDelay, protMeleeDelay, protRangeDelay, lastAction, lastThievingAttempt, lastLockPick, alchDelay, specDelay = System.currentTimeMillis(), duelDelay, teleBlockDelay, godSpellDelay, singleCombatDelay, singleCombatDelay2, reduceStat, restoreStatsDelay, logoutDelay, buryDelay, foodDelay, potDelay, lastCast = 0, buttonDelay;

	/**
	 * Booleans
	 */
	public boolean isInvisible = false, isNpc = false, jailed = false, muted = false, banned = false, canChangeAppearance = false, mageAllowed, initialized = false, disconnected = false, ruleAgreeButton = false, RebuildNPCList = false, isActive = false, isKicked = false, isSkulled = false, friendUpdate = false, newPlayer = false, hasMultiSign = false, saveCharacter = false, mouseButton = false, splitChat = false, chatEffects = true, acceptAid = false, nextDialogue = false, autocasting = false, usedSpecial = false, mageFollow = false, dbowSpec = false, craftingLeather = false, properLogout = false, secDbow = false, maxNextHit = false, ssSpec = false, vengOn = false, addStarter = false, accountFlagged = false, msbSpec = false, stopPlayerSkill, isMember, inChampionChallenge, houseOwned, debugMode = false, autoRet, goblinBow, goblinSalute, glassBox, climbRope, lean, glassWall, idea, stomp, flap, slapHead, zombieWalk, zombieDance, zombieHand, scared, bunnyHop, skillcape, snowmanDance, airGuitar, safetyFirst;

	/**
	 * Integers
	 */
	public int npcTransformId, teleGrabItem, teleGrabX, teleGrabY, duelCount, underAttackBy, underAttackBy2, wildLevel, teleTimer, respawnTimer, saveTimer = 0, teleBlockLength, poisonDelay, saveDelay, playerKilled, pkPoints, totalPlayerDamageDealt, killedBy, lastChatId = 1, privateChat, friendSlot = 0, dialogueId, randomCoffin, newLocation, specEffect, specBarId, attackLevelReq, defenceLevelReq, strengthLevelReq, rangeLevelReq, magicLevelReq, followId, skullTimer, votingPoints, nextChat = 0, talkingNpc = -1, dialogueAction = 0, dialogueTeleportId = 0, autocastId, followDistance, followId2, barrageCount = 0, delayedDamage = 0, delayedDamage2 = 0, pcPoints = 0, magePoints = 0, lastArrowUsed = -1, clanId = -1, pcDamage = 0, xInterfaceId = 0, xRemoveId = 0, xRemoveSlot = 0, tzhaarToKill = 0, tzhaarKilled = 0, waveId, frozenBy = 0, poisonDamage = 0, dialogueTeleportAction = 0, bonusAttack = 0, lastNpcAttacked = 0, champEarthWarrior, champGhoul, champGiant, champGoblin, champHobgoblin,
	        champImp, champJogre, champLesserDemon, champSkeleton, champZombie, champHuman, champFightLoaded, agilityPoints = 0, gnomeCourseLaps = 0, barbarianCourseLaps = 0, wildernessCourseLaps = 0, testQuestProgress = 0, questPoints = 0, droppedItem = -1;

	/**
	 * Strings
	 */
	public String creationDate, clanName, properName;
	 ...
	 * Doubles
	 */
	public double specAmount = 0, specAccuracy = 1, specDamage = 1, prayerPoint = 1.0;
	...
	public int[] itemPickups = new int[5];
	...
	public final int[] REDUCE_SPELL_TIME = { 250000, 250000, 250000, 500000, 500000, 500000 }; // how long does the other player stay immune to
	                                                                                           // the spell
	...
	public int playerInstance = playerId * 4;
	...
	public int bankItems[] = new int[Banking.BANK_SIZE];
	public int bankItemsN[] = new int[Banking.BANK_SIZE];
	...
	public Player(int _playerId) {
		...
		playerAppearance[12] = 1; // skin colour
		...
		teleportToX = Location.HOME[0];
		teleportToY = Location.HOME[1];
	...
	public byte cachedPropertiesBitmap[] = new byte[(Configuration.MAX_PLAYERS + 7) >> 3];
	...
	public void addNewNPC(NPC npc, Stream str, Stream updateBlock) {
		...
		str.writeBits(1, 0);
		str.writeBits(14, npc.npcType); // Change this to 14+ if can't load new NPC's
	...
	protected void appendPlayerAppearance(Stream str) {
		synchronized (this) {
			...
			if (isNpc == false) {
				...
				playerProps.writeWord(npcTransformId);
			}
	...
	public boolean inBank() {
		if (absX > 3090 && absX < 3099 && absY > 3487 && absY < 3500 || absX > 3089 && absX < 3090 && absY > 3492 && absY < 3498 || absX > 3248 && absX < 3258 && absY > 3413 && absY < 3428 || absX > 3179 && absX < 3191 && absY > 3432 && absY < 3448 || absX > 2944 && absX < 2948 && absY > 3365 && absY < 3374 || absX > 2942 && absX < 2948 && absY > 3367 && absY < 3374 || absX > 2944 && absX < 2950 && absY > 3365 && absY < 3370 || absX > 3008 && absX < 3019 && absY > 3352 && absY < 3359 || absX > 3017 && absX < 3022 && absY > 3352 && absY < 3357 || absX > 3203 && absX < 3213 && absY > 3200 && absY < 3237 || absX > 3212 && absX < 3215 && absY > 3200 && absY < 3235 || absX > 3215 && absX < 3220 && absY > 3202 && absY < 3235 || absX > 3220 && absX < 3227 && absY > 3202 && absY < 3229 || absX > 3227 && absX < 3230 && absY > 3208 && absY < 3226 || absX > 3226 && absX < 3228 && absY > 3230 && absY < 3211 || absX > 3227 && absX < 3229 && absY > 3208 && absY < 3226) {
			return true;
		}
		return false;
	}
