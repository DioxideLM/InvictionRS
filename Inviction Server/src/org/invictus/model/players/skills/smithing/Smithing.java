...
public class Smithing extends SkillHandler {
	...
	public void sendSmelting() {
		for (int j = 0; j < SMELT_FRAME.length; j++) {
			player.getPlayerAssistant().sendInterfaceItem(SMELT_FRAME[j], 150, SMELT_BARS[j]);
		}
		player.getPlayerAssistant().sendChatInterface(2400);
	...
	public void startSmelting(int barType) {
		if (canSmelt(barType)) {
			...
			if (hasOres(barType)) {
				...
				player.sendMessage("You do not have the required ores to smelt this.");
				player.getPlayerAssistant().resetVariables();
			}
		} else {
			player.sendMessage("You must have a higher smithing level to smith this.");
			player.getPlayerAssistant().resetVariables();
		}
	}
	...
	public void smelt(int barType) {
		if (player.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				player.getPlayerAssistant().addSkillXP(exp * EXPERIENCE_MULTIPLIER, SMITHING);
	...
	public boolean canSmelt(int barType) {
		for (int j = 0; j < SMELT_BARS.length; j++) {
			if (barType == SMELT_BARS[j]) {
				...
				return player.playerLevel[player.playerSmithing] >= BAR_REQS[j];
			}
		}
		return false;
	}
	...
	public void readInput(int level, String type, Client c, int amounttomake) {
		...
		if (c.getItems().getItemName(Integer.parseInt(type)).contains("Bronze")) {
			...
			CheckAddy(level, amounttomake, type);
			...
			CheckRune(level, amounttomake, type);
		}
		c.sendMessage("Item: " + type);
	}

	private void CheckIron(int level, int amounttomake, String type) {
		remove = 2351;
		...
		doaction(item, remove, removeamount, maketimes, -1, -1, xp);

	}

	private void CheckSteel(int level, int amounttomake, String type) {
		remove = 2353;
	...
	private void CheckMith(int level, int amounttomake, String type) {
		remove = 2359;
	...
	private void CheckRune(int level, int amounttomake, String type) {
		remove = 2363;
	...
	private void CheckAddy(int level, int amounttomake, String type) {
		remove = 2361;
	...
	private void CheckBronze(int level, int amounttomake, String type) {
	...
	public boolean doaction(int toadd, int toremove, int toremove2, int timestomake, int NOTUSED, int NOTUSED2, int xp) {
		...
		if (player.getItems().playerHasItem(toremove, toremove2)) {
			...
			if (maketimes > 1 && player.getItems().playerHasItem(toremove, toremove2 * 2)) {
				player.sendMessage("You make some " + player.getItems().getItemName(toadd) + "s");
			} else {
				player.sendMessage("You make a " + player.getItems().getItemName(toadd));
			}
			while (maketimes > 0) {
				if (player.getItems().playerHasItem(toremove, toremove2)) {
					player.getItems().deleteItem2(toremove, toremove2);
					if (player.getItems().getItemName(toadd).contains("bolt")) {
						player.getItems().addItem(toadd, 10);
					} else if (player.getItems().getItemName(toadd).contains("nail")) {
					...
					} else if (player.getItems().getItemName(toadd).contains("cannon")) {
						player.getItems().addItem(toadd, 4);
					} else {
						player.getItems().addItem(toadd, 1);
					}
					player.getPlayerAssistant().addSkillXP(xp * EXPERIENCE_MULTIPLIER, SMITHING);
					player.getPlayerAssistant().refreshSkill(13);
					maketimes--;
				} else {
					break;
				}
