package org.invictus.model.players.skills.mining;

import org.invictus.model.players.Client;
import org.invictus.model.players.skills.SkillHandler;
import org.invictus.util.Misc;

/**
 * @Author Sanity
 */

public class Mining extends SkillHandler {

	Client player;

	public Mining(Client player) {
		this.player = player;
	}

	private final int VALID_PICK[] = { 1265, 1267, 1269, 1273, 1271, 1275 };
	private final int[] PICK_REQS = { 1, 1, 6, 6, 21, 31, 41, 61 };
	private final int[] RANDOM_GEMS = { 1623, 1621, 1619, 1617, 1631 };
	private int oreType;
	private int exp;
	private int pickType;
	private final int EMOTE = 625;

	public void startMining(int oreType, int levelReq, int exp) {
		player.turnPlayerTo(player.objectX, player.objectY);
		if (goodPick() > 0) {
			if (player.playerLevel[player.playerMining] >= levelReq) {
				this.oreType = oreType;
				this.exp = exp;
				this.setPickType(goodPick());
				player.sendMessage("You swing your pick at the rock.");
				player.miningTimer = getMiningTimer(oreType);
				player.startAnimation(EMOTE);
			} else {
				resetMining();
				player.sendMessage("You need a mining level of " + levelReq + " to mine this rock.");
				player.startAnimation(65535);
			}
		} else {
			resetMining();
			player.sendMessage("You need a pickaxe to mine this rock.");
			player.startAnimation(65535);
			player.getPlayerAssistant().resetVariables();
		}
	}

	public void mineOre() {
		if (player.getItems().addItem(oreType, 1)) {
			player.startAnimation(EMOTE);
			player.sendMessage("You manage to mine some ore.");
			player.getPlayerAssistant().addSkillXP(exp * EXPERIENCE_MULTIPLIER, MINING);
			player.getPlayerAssistant().refreshSkill(player.playerMining);
			player.miningTimer = getMiningTimer(oreType);
			if (Misc.random(25) == 10) {
				player.getItems().addItem(RANDOM_GEMS[(int) (RANDOM_GEMS.length * Math.random())], 1);
				player.sendMessage("You find a gem!");
			}
		} else {
			player.getPlayerAssistant().resetVariables();
			player.startAnimation(65535);
		}
	}

	public void resetMining() {
		this.oreType = -1;
		this.exp = -1;
		this.setPickType(-1);
	}

	public int goodPick() {
		for (int j = VALID_PICK.length - 1; j >= 0; j--) {
			if (player.playerEquipment[player.playerWeapon] == VALID_PICK[j]) {
				if (player.playerLevel[player.playerMining] >= PICK_REQS[j])
					return VALID_PICK[j];
			}
		}
		for (int i = 0; i < player.playerItems.length; i++) {
			for (int j = VALID_PICK.length - 1; j >= 0; j--) {
				if (player.playerItems[i] == VALID_PICK[j] + 1) {
					if (player.playerLevel[player.playerMining] >= PICK_REQS[j])
						return VALID_PICK[j];
				}
			}
		}
		return -1;
	}

	public int getMiningTimer(int ore) {
		int time = Misc.random(5);
		if (ore == 451) {
			time += 4;
		}
		return time;
	}

	public int getPickType() {
		return pickType;
	}

	public void setPickType(int pickType) {
		this.pickType = pickType;
	}

}