...
public class Potions {
	...
	public Potions(Client player) {
		this.player = player;
	}

	public void handlePotion(int itemId, int slot) {
		if (player.duelRule[5]) {
			player.sendMessage("You may not drink potions in this duel.");
			return;
		}
		if (System.currentTimeMillis() - player.potDelay >= 1500) {
			...
			switch (itemId) {
			case 6685: // brews
				drinkSaradominBrew(itemId, 6687, slot);
				break;
			case 6687:
	...
	public void restoreStats() {
		for (int j = 0; j <= 6; j++) {
			...
			if (player.playerLevel[j] < player.getLevelForXP(player.playerXP[j])) {
				...
				player.getPlayerAssistant().refreshSkill(j);
				player.getPlayerAssistant().setSkillLevel(j, player.playerLevel[j], player.playerXP[j]);
			}
		}
	}
	...
	public void drinkSaradominBrew(int itemId, int replaceItem, int slot) {
		...
		for (int tD : toDecrease) {
			...
			player.getPlayerAssistant().refreshSkill(tD);
			player.getPlayerAssistant().setSkillLevel(tD, player.playerLevel[tD], player.playerXP[tD]);
		}
	...
	public void enchanceStat(int skillID, boolean sup) {
		...
		player.getPlayerAssistant().refreshSkill(skillID);
	}
