package org.invictus.model.players.content.minigames.fightpits;
...
public class FightPits {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if (c.getPlayerAssistant().inPitsWait()) {
					c.getPlayerAssistant().sendString("Next Game Begins In : " + ((gameStartTimer * 3) + (gameTime * 3)) + " seconds.", 6570);
					c.getPlayerAssistant().sendString("Champion: " + pitsChampion, 6572);
	...
	public void removePlayerFromPits(int playerId) {
		for (int j = 0; j < playerInPits.length; j++) {
			if (playerInPits[j] == playerId) {
				...
				c.getPlayerAssistant().movePlayer(2399, 5173, 0);
				playerInPits[j] = -1;
				playersRemaining--;
				c.inPits = false;
				break;
	...
	public void addToPitsGame(int playerId) {
		...
		c.getPlayerAssistant().movePlayer(2392 + Misc.random(12), 5139 + Misc.random(25), 0);
		c.inPits = true;
	}
}