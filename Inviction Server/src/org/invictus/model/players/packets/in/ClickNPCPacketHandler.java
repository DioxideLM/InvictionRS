...
import org.invictus.model.players.CombatAssistant;
import org.invictus.model.players.packets.PacketType;

/**
 * Handles the packet for clicking NPC's.
 */
public class ClickNPCPacketHandler implements PacketType {
	...
	public void processPacket(Client c, int packetType, int packetSize) {
		...
		case ATTACK_NPC:
			...
			if (c.getCombat().correctBowAndArrows() < c.playerEquipment[c.playerArrows] && CombatAssistant.CORRECT_ARROWS && usingBow && !c.getCombat().usingCrystalBow() && c.playerEquipment[c.playerWeapon] != 9185) {
		...
		case MAGE_NPC:
			...
			if (castingSpellId == 1171) { // crumble undead
				for (int npc : NPCHandler.UNDEAD_NPCS) {
